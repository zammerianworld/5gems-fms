...
export default function Login() {
  ...
  const [companyName, setCompanyName] = useState(localStorage.getItem('ds_company_name') || 'Dragon Speed Trucking')
    ...
    if (superName && superHash && email.trim().toLowerCase() === superName.toLowerCase()) {
      ...
      if (hashHex === superHash) {
        ...
        sessionStorage.setItem('superuser_session', JSON.stringify({
          ...
          email: 'superuser@dragonspeed.internal',
        }))
        ...
        await logAttempt({ user_name: 'Superuser', user_role: 'superuser', email: 'superuser@dragonspeed.internal', status: 'success' })
  ...
  return (
    <div style={{
      ...
      {/* Glow blobs — cheap, static (no animation lib, no bundle cost).
          These sit behind everything and give the glass card something
          to actually show through. */}
      <div style={{ position: 'absolute', width: 320, height: 320, borderRadius: '50%', background: '#f17200', opacity: 0.28, filter: 'blur(70px)', top: -80, left: -80, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', width: 280, height: 280, borderRadius: '50%', background: '#f17200', opacity: 0.2, filter: 'blur(70px)', bottom: -70, right: -70, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', width: 200, height: 200, borderRadius: '50%', background: '#fff', opacity: 0.06, filter: 'blur(60px)', top: '38%', left: '62%', pointerEvents: 'none' }} />
      ...
      <div style={{ width: '100%', maxWidth: 380, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ margin: '0 auto 14px', maxWidth: 280 }}>
            {logoUrl
              ...
              : <div style={{ width: 72, height: 72, background: 'linear-gradient(145deg, rgba(255,255,255,0.14), rgba(255,255,255,0.04))', borderRadius: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36, margin: '0 auto', boxShadow: '0 6px 14px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.1)' }}><span>🐉</span></div>}
          </div>
        ...
        <div className="ds-login-glass" style={{
          ...
          boxShadow: '0 0 0 1px rgba(255,255,255,0.04) inset, 0 24px 50px rgba(0,0,0,0.45), 0 4px 10px rgba(0,0,0,0.3)',
        }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={loading}
              style={{
                ...
                background: '#f17200', color: '#3d1c00', fontSize: 14, fontWeight: 500,
                cursor: loading ? 'default' : 'pointer', opacity: loading ? 0.7 : 1,
                boxShadow: '0 4px 16px rgba(241,114,0,0.4)',
              }}>
