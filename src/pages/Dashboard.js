        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>
              {unpaidInvoices.length} invoice{unpaidInvoices.length > 1 ? 's' : ''} · ₱{fmt(unpaidInvoices.reduce((s, i) => s + (i.total_sales_net || 0) * (i.is_vat ? 1.12 : 1), 0))}
            </div>
          </div>
              ...
              return (
                <div key={inv.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '0.5px solid var(--border)' }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontSize: 12, fontFamily: 'var(--mono)', fontWeight: 500 }}>₱{fmt((inv.total_sales_net || 0) * (inv.is_vat ? 1.12 : 1))}</div>
