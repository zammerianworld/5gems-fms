...
import DriversPayroll from '../components/DriversPayroll'
import Tenure13thMonth from '../components/Tenure13thMonth'
...
const EMPTY_CA = {
  employee_id: '', driver_id: '', date: '', amount: '', description: '', type: 'advance'
}
...
// Support Staff run weekly instead of semi-monthly — cutoff lands on the
// most recent Saturday (or today, if today is Saturday).
const currentWeeklyCutoffDate = () => {
  const now = new Date()
...
export default function Employees() {
  ...
  // category. 'drivers' renders the separate trip-based engine entirely.
  const [group, setGroup] = useState('admin')

  // ── DATA STATE ──────────────────────────────────────────────────────────
  const [employees, setEmployees] = useState([])
  const [allEmployees, setAllEmployees] = useState([]) // unscoped by category — CA ledger needs everyone, not just the active Admin/Support toggle
  ...
  const [pendingPrintFn, setPendingPrintFn] = useState(null) // (sigs) => void — set before opening sigDialog when the caller isn't the register-wide browser-print flow
  ...
  const [thirteenthGroup, setThirteenthGroup] = useState('admin') // 'admin' | 'tenure'
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      const { data } = await supabase.from('payroll_employees').select('*').eq('category', group).order('full_name')
      setEmployees(data || [])
    } catch (e) { console.warn('fetchEmployees:', e.message) }
  }, [group])

  // Re-scope everything to the newly selected group — different roster,
  // different cutoff cadence for Support Staff (weekly vs semi-monthly).
  useEffect(() => {
    if (group === 'drivers') return // Drivers tab manages its own fetching entirely
    const cutoff = group === 'support' ? currentWeeklyCutoffDate() : currentCutoffDate()
  ...
  const fetchCaRecords = useCallback(async () => {
    try {
      const { data } = await supabase
        ...
        .select('*, payroll_employees(full_name), drivers(driver_name)')
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('payroll_employees').select('id,full_name,category').order('full_name').then(r => setAllEmployees(r.data || [])),
      supabase.from('drivers').select('id,driver_name').eq('active', true).order('driver_name').then(r => setAllDrivers(r.data || [])),
  ...
  const handle13thPrint = () => {
    ...
    doc.text('FLEET MANAGEMENT SYSTEM', doc.internal.pageSize.width/2, 15, { align:'center' })
  ...
  const printSinglePayslip = (entry) => {
    setPendingPrintFn(() => (sigs) => doPrintSinglePayslip(entry, sigs))
    setSigDialog(true)
  }
  ...
  const doPrintSinglePayslip = (entry, sigs) => {
    ...
    const doc = buildPayslipDoc({
      ...
      salary: p(entry.basic_salary),
      overtime: p(entry.overtime_pay),
      // rest_day_duty and salary_adjustment don't have their own line in this
      // layout — folded into Allowance so nothing drops out of the net total.
      allowance: p(entry.allowance) + p(entry.rest_day_duty) + p(entry.salary_adjustment),
      deductions: [
        { label: 'SSS Premium', amount: p(entry.sss_premium) },
        { label: 'Philhealth', amount: p(entry.philhealth_premium) },
        { label: 'Pag-ibig Fund', amount: p(entry.hdmf_premium) },
  ...
  const handleSaveCa = async () => {
    if ((!caForm.employee_id && !caForm.driver_id) || !caForm.date || !caForm.amount) {
      showToast('Person, date, and amount are required.', 'error'); return
    }
    setSaving(true)
    const payload = {
      employee_id: caForm.employee_id || null,
      driver_id: caForm.driver_id || null,
      date: caForm.date,
  ...
  // ── FILTERED CA ─────────────────────────────────────────────────────────
  const filteredCa = caFilterEmp === 'all' ? caRecords : caRecords.filter(r =>
    caFilterEmp.startsWith('emp:') ? r.employee_id === caFilterEmp.slice(4) : r.driver_id === caFilterEmp.slice(7))
    ...
    <div style={{ padding: '16px 12px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
            {activeTab === 'payroll'
              ? (group === 'support' ? 'Support Staff — weekly payroll' : 'Admin — semi-monthly payroll')
                      ...
                      <div key={e.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 14px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                          ...
                          <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 8 }}>
                            {isAdmin && <button onClick={() => printSinglePayslip(e)} style={{ padding: '6px 10px', background: '#334155', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}>🖨️</button>}
                            {isAdmin && !cutoffLocked && (
                              <>
                              <button onClick={() => {
                            ...
                            {isAdmin && (
                              <td style={TD}>
                                <div style={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
                                  <button onClick={() => printSinglePayslip(e)} style={ActionBtn('#334155')}>🖨️</button>
                                  {!cutoffLocked && (
                                    <>
                                    <button onClick={() => {
                                      setEditingEntryId(e.id); editingEntryIdRef.current = e.id
        ...
        <>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
            <select value={caFilterEmp} onChange={e => setCaFilterEmp(e.target.value)}
              ...
              <optgroup label="Admin">{allEmployees.filter(e => (e.category || 'admin') === 'admin').map(e => <option key={e.id} value={`emp:${e.id}`}>{e.full_name}</option>)}</optgroup>
              <optgroup label="Support Staff">{allEmployees.filter(e => e.category === 'support').map(e => <option key={e.id} value={`emp:${e.id}`}>{e.full_name}</option>)}</optgroup>
              <optgroup label="Drivers">{allDrivers.map(d => <option key={d.id} value={`driver:${d.id}`}>{d.driver_name}</option>)}</optgroup>
            </select>
          </div>

          {/* Per-person balance summary cards — Admin + Support + Drivers together */}
          {(() => {
            const people = [
              ...allEmployees.map(e => ({ key: `emp:${e.id}`, id: e.id, name: e.full_name, kind: 'emp' })),
              ...allDrivers.map(d => ({ key: `driver:${d.id}`, id: d.id, name: d.driver_name, kind: 'driver' })),
            ].filter(p => caFilterEmp === 'all' || p.key === caFilterEmp)
            return people.map(person => {
              const recs = caRecords.filter(r => person.kind === 'emp' ? r.employee_id === person.id : r.driver_id === person.id)
              ...
              const totalPayrollDed = person.kind === 'emp'
                ? allPayrollEntries.filter(pe => pe.employee_id === person.id).reduce((s, pe) => s + (parseFloat(pe.cash_advance_deduction) || 0), 0)
                : 0 // driver payroll deductions tracked within the Drivers tab's own entries — not yet cross-referenced here
              const balance = Math.max(0, totalAdv - totalLedgerPay - totalPayrollDed)
              if (totalAdv === 0 && balance === 0) return null
              return (
                <div key={person.key} style={{ background: 'var(--surface)', border: `1px solid ${balance > 0 ? '#fca5a5' : 'var(--border)'}`, borderRadius: 8, padding: '12px 16px', marginBottom: 12 }}>
                  <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 8 }}>{person.name}{person.kind === 'driver' && <span className="badge" style={{ fontSize: 9, marginLeft: 6, background: 'rgba(139,92,246,0.12)', color: '#6d28d9' }}>DRIVER</span>}</div>
            ...
            <table style={{ width: '100%', minWidth: 600, borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {filteredCa.length === 0 ? (
                  ...
                  const empIds = new Set(filteredCa.filter(r => r.employee_id).map(r => r.employee_id))
                  const payrollRows = allPayrollEntries
                    .filter(pe => empIds.has(pe.employee_id) && (parseFloat(pe.cash_advance_deduction) || 0) > 0)
                    .map(pe => {
                      const emp = allEmployees.find(e => e.id === pe.employee_id)
                      return { key: 'pr_' + pe.employee_id + '_' + pe.cutoff_date, personKey: 'emp:' + pe.employee_id, empName: emp?.full_name || '—', date: pe.cutoff_date, type: 'payment', amount: parseFloat(pe.cash_advance_deduction) || 0, description: 'Payroll deduction (' + pe.cutoff_date + ')', source: 'payroll', originalRow: null }
                    })
                  const ledgerRows = filteredCa.map(r => ({
                    key: 'ca_' + r.id,
                    personKey: r.employee_id ? 'emp:' + r.employee_id : 'driver:' + r.driver_id,
                    empName: r.payroll_employees?.full_name || r.drivers?.driver_name || '—',
                    date: r.date, type: r.type, amount: parseFloat(r.amount) || 0, description: r.description || '', source: 'ledger', originalRow: r,
                  }))
                  const allRows = [...ledgerRows, ...payrollRows].sort((a, b) => a.date < b.date ? -1 : a.date > b.date ? 1 : 0)
                  const runningMap = {}
                  return allRows.map(r => {
                    if (runningMap[r.personKey] === undefined) runningMap[r.personKey] = 0
                    if (r.type === 'advance') runningMap[r.personKey] += r.amount
                    else runningMap[r.personKey] -= r.amount
                    const runBal = Math.max(0, runningMap[r.personKey])
      ...
      {showCaForm && (
        <Modal title={editingCaId ? 'Edit CA Record' : 'Add CA Record'} onClose={() => { setShowCaForm(false); setEditingCaId(null); setCaForm(EMPTY_CA) }}>
          <div style={{ display: 'grid', gap: 14 }}>
            <FormRow label="For *">
              <select
                value={caForm.driver_id ? `driver:${caForm.driver_id}` : caForm.employee_id ? `emp:${caForm.employee_id}` : ''}
                onChange={e => {
                  const [kind, id] = e.target.value.split(':')
                  setCaForm(f => ({ ...f, employee_id: kind === 'emp' ? id : '', driver_id: kind === 'driver' ? id : '' }))
                }} style={INPUT}>
      ...
      <SignatoryDialog open={sigDialog} onClose={() => setSigDialog(false)}
        onPrint={(sigs) => {
          ...
          else handlePrint(sigs)
        }} settings={settings} profile={profile} docType="Payroll" />
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: 'var(--bg)', padding: 3, borderRadius: 8, width: 'fit-content' }}>
            {[{ key: 'admin', label: 'Admin' }, { key: 'tenure', label: 'Drivers & Support Staff' }].map(o => (
              <button key={o.key} onClick={() => setThirteenthGroup(o.key)} style={{
                ...
                background: thirteenthGroup === o.key ? 'var(--surface)' : 'transparent',
                color: thirteenthGroup === o.key ? 'var(--text)' : 'var(--muted)',
                boxShadow: thirteenthGroup === o.key ? '0 1px 2px rgba(0,0,0,0.08)' : 'none',
              }}>{o.label}</button>
            ))}
          </div>
          {thirteenthGroup === 'tenure' ? (
            <Tenure13thMonth isAdmin={isAdmin} profile={profile} showToast={showToast} settings={settings} />
          ) : (
          <div>
