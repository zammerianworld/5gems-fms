...
export default function Cashflow() {
  ...
  const invoiceCashIn = monthPaid.reduce((s, i) => s + (parseFloat(i.actual_amount_credited) || (i.total_sales_net || 0) * (i.is_vat ? 1.10 : 0.98)), 0)
  ...
  const expectedCashIn = unpaidInvoices.reduce((s, i) => s + (i.total_sales_net || 0) * (i.is_vat ? 1.10 : 0.98), 0)
  ...
  const cashPosition = cashMonths.map(mo => {
    const cashIn = invoices.filter(i => i.status === 'Paid' && i.date_credited?.slice(0, 7) === mo)
      .reduce((s, i) => s + (parseFloat(i.actual_amount_credited) || (i.total_sales_net || 0) * (i.is_vat ? 1.10 : 0.98)), 0)
  ...
  const handleSavePDF = () => {
    ...
    autoTable(doc, {
      ...
      body: monthPaid.map(i => [fmtDate(i.invoice_date), i.invoice_no, i.client, fmt(parseFloat(i.actual_amount_credited) || (i.total_sales_net || 0) * (i.is_vat ? 1.10 : 0.98))]),
  ...
  const handleExport = () => {
    ...
    if (exportFormat === 'excel') {
      ...
      if (isDetail) monthPaid.forEach(i => { html += `<tr style="background:#F0FFF4"><td style="padding-left:20px">${i.invoice_no} · ${i.client}</td><td style="text-align:right">₱${f2(parseFloat(i.actual_amount_credited)||(i.total_sales_net||0)*(i.is_vat?1.10:0.98))}</td></tr>` })
      ...
      if (isDetail) monthPaid.forEach(i => { doc.setFontSize(7.5); doc.setFont('helvetica','normal'); doc.setTextColor(80); doc.text(`  ${i.invoice_no} · ${i.client}`, 20, y); doc.text(`PHP ${f2(parseFloat(i.actual_amount_credited)||(i.total_sales_net||0)*(i.is_vat?1.10:0.98))}`, W-14, y, { align:'right'}); y+=5 })
                ...
                <tbody>
                  {monthPaid.map(i => (
                    <tr key={i.id}>
                      ...
                      <td className="text-right mono" style={{ fontWeight: 500, color: 'var(--success)' }}>₱{fmt(parseFloat(i.actual_amount_credited) || (i.total_sales_net || 0) * (i.is_vat ? 1.10 : 0.98))}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot><tr>
